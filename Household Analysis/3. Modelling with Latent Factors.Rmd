...
famd_components <- as.data.frame(famd_result$ind$coord[, 1:3])
colnames(famd_components) <- paste0("FAMD_", 1:3)
...
MISKIN_KAKO <- factor(
  ifelse(data_susenas$MISKIN_KAKO == "1", "MISKIN", "TIDAK MISKIN"),
...
# Assuming 'famd_result' and 'df_model_raw' are available.
...
MISKIN_KAKO <- factor(
  ifelse(df_model_raw$MISKIN_KAKO == "1", "Poor", "NotPoor"),
...
```{r}
# # --- Section 5.1: Logistic Regression on FAMD Components ---
# 
# # 1. Load necessary libraries
# library(caret)
# library(dplyr)
# 
# cat("\n--- Training Logistic Regression on FAMD Data ---\n")
# 
# # 2. Prepare Training Data
# # Start with the SMOTE-balanced data prepared from FAMD components.
# train_lr_famd <- train_smote_universal
# 
# # 3. Apply Preprocessing Filters for Consistency
# # NOTE: These filters are unlikely to remove any of the 3 FAMD components,
# # but we include the steps to maintain a consistent workflow.
# 
# # 3.1 Apply Near-Zero Variance (NZV) filter
# nzv_cols <- nearZeroVar(train_lr_famd, saveMetrics = FALSE)
# if (length(nzv_cols) > 0) {
#   train_lr_famd <- train_lr_famd[, -nzv_cols]
#   cat("Removed", length(nzv_cols), "near-zero variance predictors.\n")
# } else {
#   cat("No NZV predictors found.\n")
# }
# 
# # 3.2 Apply Correlation filter (FAMD components should be uncorrelated)
# numeric_vars <- select(train_lr_famd, where(is.numeric))
# if (ncol(numeric_vars) > 1) {
#   corr_matrix <- cor(numeric_vars)
#   high_corr_indices <- findCorrelation(corr_matrix, cutoff = 0.9)
#   if (length(high_corr_indices) > 0) {
#     train_lr_famd <- train_lr_famd[, -high_corr_indices]
#     cat("Removed", length(high_corr_indices), "highly correlated predictors.\n")
#   } else {
...
# final_lr_famd_predictors <- setdiff(names(train_lr_famd), "MISKIN_KAKO")
# test_lr_famd <- test_data_universal %>%
#   select(all_of(final_lr_famd_predictors), MISKIN_KAKO)
# 
# # 5. Train and Evaluate the Model
# # Call our reusable function. 'glm' has no parameters to tune.
# set.seed(123) # for reproducibility
# model_lr_famd <- train_and_evaluate_model(
#   train_data   = train_lr_famd,
#   test_data    = test_lr_famd,
#   model_name   = "Logistic Regression (FAMD)",
#   method_caret = "glm"
# )
# 
# cat("\nLogistic Regression (FAMD) model training and evaluation complete.\n")
# 
# # 6. Generate Updated Comparison Report
# # This will now include the results from our new model.
# final_summary_table <- create_comparison_table()
# print(final_summary_table)
# 
# # Plot the updated final performance and diagnostic plots
...
```{r}
# --- Section 5.2: Decision Tree on FAMD Components ---
...
```{r}
# --- Section 5.3: Random Forest on FAMD Components ---
...
```{r}
# --- Section 5.4: XGBoost on FAMD Components ---
...
saveRDS(
  comparison_tbl,
  file = file.path(output_dir, "comparison_table_HH_prior.rds")
)
...
saveRDS(
  cv_accuracy_df,
  file = file.path(output_dir, "cv_accuracy_data_HH_prior.rds")
)
...
saveRDS(
  roc_data_df,
  file = file.path(output_dir, "roc_curve_data_HH_prior.rds")
)

...
## 3. Individual data and flag miskin
...
## 4. Household dataset and flag miskin
...
### 6.6 Schools and Colleges Attended (even though not finished)
...
# 3. Create a support variable for data_kor_ind
...
# 4. Create the pooled_summary first
...
# 5. Perform a LEFT JOIN so that the pooled variable is added at the end
data_kor_rt <- data_kor_rt %>%
  left_join(pooled_summary, by = "URUT")
```

```{r}
# List of pooled variables
pooled_vars <- c(
...
# Calculate the count and percentage of NAs.
...
### 6.7 Highest qualification held
...
# 2. Add a helper variable to data_kor_ind
data_kor_ind <- data_kor_ind %>%
  mutate(
...
# 3. Pooling to the household level.
...
# 4. Merge the pooled results into data_kor_rt and handle the NAs.
data_kor_rt <- data_kor_rt %>%
  left_join(cert_summary, by = "URUT") %>%
  mutate(
    # Handle the NAs with a logical default value
    ...
    # Convert has_tertiary into a factor with labels
    has_tertiary = factor(
      has_tertiary,
      levels = c(0, 1),
...
```{r}
# List of pooled variables from R614.
cert_vars <- c(
  "n_no_cert",
  "highest_cert_level",
  "highest_cert_group",
...
# Display the summary with neat rounding.
...
### 6.11 Using financial products and services
...
# --- create the frequency table for the new factor ---

---
title: "Factor Analysis with only 4 dimension"
author: "Ahmad Risal"
date: "2025-06-06"
...
data_famd <- data_susenas %>%
  select(-c(R102, MISKIN_KAKO))
```

cek variable used in famd

```{r}
# Corrected command
table(sapply(data_famd, function(col) class(col)[1]))
```
...
famd_result$var$contrib[, 1:4]        # Numeric variables
famd_result$quali.var$contrib[, 1:4]  # Categorical variables
```
...
# Export to Excel
# write_xlsx(as.data.frame(eigen_values), "famd_eigen_values_4 Dims.xlsx")
```

```{r}
# Extract contribution table
var_contrib <- famd_result$var$contrib[, 1:4]
...
clustering_data <- as.data.frame(famd_result$ind$coord[, 1:4])
...
```{r}
final_clustered_data <- filtered_data
...
important_vars <- c(
  "R301", "R407", "MISKIN_KAKO", "R610", "R612", "R614", "R607", "R608", 
  "R609", "R615", "R616", "R701", "R702", "R2201A2_RT", "R706", "R707", 
  "R708", "R709", "R705", "R1808_RT", "R1817_RT", "R2001C_RT", "R2001F_RT", 
  "R808", "R801", "R802", "R404", "R1207", "R1209", "R1206"
)
...
ggplot(cluster_sizes, aes(x = cluster, y = n, fill = cluster)) +
  ...
  labs(
    ...
    subtitle = "Total Observation: 27205",
    x = "Cluster",
...
# Assume 'mds_df' is your data frame with the columns Dim1, Dim2, and Cluster

ggplot(mds_df, aes(x = Dim1, y = Dim2, color = Cluster, fill = Cluster)) +
  
  # Draw cluster ellipses with a clear outline
  # The main changes are on this line:
  # - Removing 'colour = NA'
  # - Adding 'size = 1' to make the line thicker
  # - Slightly increasing the alpha for the fill to make it more visible
  ...
  # Dark2 for the outlines (points and ellipse outline)
  # Pastel2 for the fill (ellipse fill)
  scale_color_brewer(palette = "Dark2") +
  scale_fill_brewer(palette = "Pastel2") +
